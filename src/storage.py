...
class RolloutStorage(object):
    def __init__(self, max_num_steps, obs_dim, act_dim):
        self.max_num_steps = max_num_steps
        self.obs_dim = obs_dim
        self.act_dim = act_dim

        self.obs = np.zeros((max_num_steps + 1, obs_dim), dtype="float32")
        self.actions = np.zeros((max_num_steps, act_dim), dtype="float32")
        self.value_preds = np.zeros((max_num_steps + 1,), dtype="float32")
        self.returns = np.zeros((max_num_steps + 1,), dtype="float32")
        self.action_log_probs = np.zeros((max_num_steps,), dtype="float32")
        self.rewards = np.zeros((max_num_steps,), dtype="float32")

        self.masks = np.ones((max_num_steps + 1,), dtype="bool")
    ...
    def sample_batch(
        ...
        mini_batch_size = self.step // num_mini_batch
        sampler = BatchSampler(
            sampler=RandomSampler(range(self.step)),
...
class RolloutStorageOG(object):
    ...
    def sample_batch(self,
        ...
        # calculate return and advantage first
        self.compute_returns(next_value, gamma, gae_lambda)
        # print(f"Storage Returns: {self.returns}")
        advantages = self.returns[:-1] - self.value_preds[:-1]
