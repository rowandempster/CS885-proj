...
class PaddleSAC(parl.Algorithm):
    def __init__(
        self, model, gamma=None, tau=None, alpha=None, actor_lr=None, critic_lr=None
    ):
        """SAC algorithm
        Args:
        ...
        self.actor_optimizer = paddle.optimizer.Adam(
            learning_rate=actor_lr, parameters=self.model.actor_model.parameters()
        )
        self.critic_optimizer = paddle.optimizer.Adam(
            learning_rate=critic_lr, parameters=self.model.critic_model.parameters()
        )
    ...
    def _critic_learn(self, obs, action, reward, next_obs, terminal):
        with paddle.no_grad():
            ...
            q1_next, q2_next = self.target_model.critic_model(next_obs, next_action)
            target_Q = paddle.minimum(q1_next, q2_next) - self.alpha * next_log_pro
            terminal = paddle.cast(terminal, dtype="float32")
            target_Q = reward + self.gamma * (1.0 - terminal) * target_Q
