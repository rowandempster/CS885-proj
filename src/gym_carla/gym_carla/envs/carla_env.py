...
class CarlaEnv(gym.Env):
    ...
    def __init__(self, params):
        ...
        self._make_carla_client('b411fdef299c', self.port)
        ...
        self.distances = [1.,5.,10.]#,15.,20.,25.,30.,35.]
            ...
            try:
                ...
                if self.task_mode == 'Straight':
                    ...
                    if self.code_mode == 'train':
                        ...
                        # self.route_deterministic_id = (
                        #     self.route_deterministic_id + 1) % 50
                ...
                # if  self.task_mode == 'Left_turn':
                #     if self.code_mode == 'train':
                #         self.route_id = self.route_deterministic_id
                #         self.route_deterministic_id = (self.route_deterministic_id + 1) % len(self.starts)
                #     elif self.code_mode == 'test':
                #         self.route_id = self.route_deterministic_id
                #         self.route_deterministic_id = (self.route_deterministic_id + 1) % len(self.starts)
                ...
                #self.rc.localizeAlongPath(self._xRefArr,self._yRefArr,xStart,yStart) 
                ...
                print("Position Error Vec",pos_err_vec)

                self.state_info['velocity_t'] = v_t
                print("Velocity_t",self.state_info['velocity_t'])
                self.state_info['acceleration_t'] = a_t
                print("Acceleration_t",self.state_info['acceleration_t'])
                self.state_info['ego_heading'] = ego_heading
                self.state_info['delta_yaw_t'] = delta_yaw
                print("Delta_yaw_t",self.state_info['delta_yaw_t'])
                self.state_info['dyaw_dt_t'] = dyaw_dt
                print("Dyaw_dt_t",self.state_info['dyaw_dt_t'])
    ...
    def step(self,action):
        try:
            ...
            self._yawRefArr,self._vRefArr = current_action 
            
            #throttle_or_brake, steer = current_action
            #listener2.subscribe(self.rc.receive_message)
        ...
        #         brake = 0 #-throttle_or_brake
            ...
            self.rc.getFeedback(transform,velocity,ct,self._xRefArr,self._yRefArr,self._yawRefArr,self._vRefArr)

            throttle = self.rc._control.throttle
            steer = self.rc._control.steer
            # throttle = th
            # steer = str
            brake = self.rc._control.brake
            ...
            # # print("Current_action",current_action,":",throttle,steer)
            # current_action[0] = th
            # current_action[1] = str
            ...
            # Localize ego vehicle
            #self.rc.localizeAlongPath(self._xRefArr,self._yRefArr,ego_x,ego_y)            
