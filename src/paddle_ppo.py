...
class PaddlePPO(parl.Algorithm):
    def __init__(self,
                 model,
                 clip_param,
                 value_loss_coef,
    ...
    def sample(self, obs):
        ...
        action_log_prob = dist.log_prob(x_t)
        action_log_prob -= paddle.log((1 - action.pow(2)) + 1e-6)
        action_log_prob = paddle.sum(action_log_prob, axis=-1, keepdim=True)
